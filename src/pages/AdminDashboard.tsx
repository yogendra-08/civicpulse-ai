...
export function AdminDashboard() {
  ...
  const deptPerformance = useMemo(() => {
    ...
    complaints.forEach((c) => {
      const name = c.departmentName || c.departmentId || t('admin.unassigned');
  ...
  if (!complaints) return <DashboardLayout><LoadingOverlay label={t('admin.loadingDashboard')} /></DashboardLayout>;

  return (
    <DashboardLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 animate-fade-in">
        <div>
          <h1 className="text-2xl font-extrabold text-navy-900">{t('admin.adminDashboard')}</h1>
          <p className="text-slate-500 mt-1">{user?.role === 'admin' ? user.municipality : ''}</p>
        </div>
        <Link to="/admin/analytics" className="btn-primary text-sm">
          <BarChart3 className="h-4 w-4" /> {t('admin.viewAnalytics')}
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        ...
        <StatCard icon={Clock} label={t('dashboard.stats.pending')} value={stats.pending} color="bg-blue-50 text-blue-600" />
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-bold text-navy-900">{t('admin.departmentPerformance')}</h2>
          </div>
            ...
            {deptPerformance.map((d) => (
              <div key={d.id}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs text-slate-500">{t('admin.resolvedOfTotal', { resolved: d.resolved, total: d.total })}</span>
                </div>
                ...
                <div className="text-xs text-slate-400 mt-0.5">{t('admin.resolutionRate', { rate: d.rate })}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-bold text-navy-900">{t('admin.wardPerformance')}</h2>
          </div>
              ...
              <div key={w.ward} className="flex items-center justify-between p-2.5 rounded-lg hover:bg-slate-50 transition">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-slate-400">{t('admin.complaintCountResolved', { total: w.total, resolved: w.resolved })}</div>
                  </div>
                </div>
      ...
      <div className="card p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-navy-900">{t('admin.recentComplaints')}</h2>
          </div>
          <div className="relative">
            ...
            <input
              ...
              placeholder={t('admin.searchRecent')}
              className="input pl-9 py-2 text-sm w-full sm:w-48"
            />
          </div>
        </div>
        {filteredRecent.length === 0 ? (
          <p className="text-sm text-slate-400 text-center py-8">{t('admin.noMatchingComplaints')}</p>
        ) : (
