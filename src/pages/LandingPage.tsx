    ...
    <div className="min-h-screen bg-white">
      ...
      <section id="how" className="bg-slate-50 border-y border-slate-200 py-16 lg:py-24">
        <div className="section-container">
          <div className="text-center max-w-2xl mx-auto mb-12">
            <div className="text-xs font-bold uppercase tracking-wider text-gov-600 mb-2">{t('landing.processEyebrow')}</div>
            <h2 className="text-3xl lg:text-4xl font-extrabold text-navy-900">{t('landing.processTitle')}</h2>
            <p className="mt-4 text-slate-600">{t('landing.processDescription')}</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {workflow.map((w, i) => (
              <div key={w.step} className="relative card p-6 animate-fade-in" style={{ animationDelay: `${i * 100}ms` }}>
                ...
                <div className="text-xs font-bold text-slate-300 mb-1">{t('landing.step', { step: w.step })}</div>
      ...
      <section id="benefits" className="section-container py-16 lg:py-24">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
          <div>
            <div className="text-xs font-bold uppercase tracking-wider text-gov-600 mb-2">{t('landing.forCitizens')}</div>
            <h2 className="text-3xl font-extrabold text-navy-900 mb-6">{t('landing.citizensTitle')}</h2>
          ...
          <div>
            <div className="text-xs font-bold uppercase tracking-wider text-saffron-600 mb-2">{t('landing.forMunicipalities')}</div>
            <h2 className="text-3xl font-extrabold text-navy-900 mb-6">{t('landing.municipalitiesTitle')}</h2>
            <div className="space-y-4">
      ...
      <section className="bg-navy-900 text-white py-16 lg:py-20 relative overflow-hidden">
        ...
        <div className="relative section-container text-center max-w-2xl mx-auto">
          <h2 className="text-3xl lg:text-4xl font-extrabold text-balance">{t('landing.ctaTitle')}</h2>
          <p className="mt-4 text-navy-200 text-lg">{t('landing.ctaDescription')}</p>
          <div className="mt-8 flex flex-col sm:flex-row justify-center gap-3">
            <Link to="/login" className="btn-accent text-base px-7 py-3.5">
              {t('landing.accessPortal')} <ArrowRight className="h-5 w-5" />
            </Link>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-navy-950 text-navy-300 py-12">
        <div className="section-container">
          <div className="grid md:grid-cols-4 gap-8 mb-8">
            <div className="md:col-span-2">
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <div>
                  <div className="font-extrabold text-white text-lg">{t('app.title')}</div>
                  <div className="text-[10px] uppercase tracking-wider text-navy-400">{t('app.portal')}</div>
                </div>
              </div>
              <p className="text-sm text-navy-400 max-w-sm leading-relaxed">{t('landing.footerDescription')}</p>
            </div>
            <div>
              <h4 className="text-sm font-bold text-white mb-3">{t('landing.portal')}</h4>
              <ul className="space-y-2 text-sm">
                <li><Link to="/login" className="hover:text-white transition">{t('landing.citizenLogin')}</Link></li>
                <li><Link to="/login" className="hover:text-white transition">{t('landing.officerLogin')}</Link></li>
                <li><Link to="/login" className="hover:text-white transition">{t('landing.adminLogin')}</Link></li>
              </ul>
            </div>
            <div>
              <h4 className="text-sm font-bold text-white mb-3">{t('landing.support')}</h4>
              <ul className="space-y-2 text-sm">
                <li className="flex items-center gap-2"><Headphones className="h-4 w-4" /> {t('landing.helpline')}</li>
                <li className="flex items-center gap-2"><MapPin className="h-4 w-4" /> {t('landing.municipalHq')}</li>
                <li className="flex items-center gap-2"><Building2 className="h-4 w-4" /> {t('landing.smartGovernanceDept')}</li>
              </ul>
            </div>
          </div>
          <div className="border-t border-navy-800 pt-6 flex flex-col sm:flex-row justify-between items-center gap-3 text-xs text-navy-400">
            <div>{t('landing.copyright2026')}</div>
            <div className="flex gap-4">
              <span>{t('footer.privacy')}</span>
              <span>{t('footer.terms')}</span>
              <span>{t('landing.accessibility')}</span>
            </div>
          </div>
        </div>
      </footer>
